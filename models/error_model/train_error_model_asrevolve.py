...
def main(args):
  ...
  labels, texts = generate_asrevolve_training_data_from_hypotheses_file(hypotheses_path)
  
  texts, labels = randomize(texts, labels)
  ...
  print('train_size: {} dev_size: {}'.format(len(train_labels),len(dev_labels)))

  estimator = HuggingFaceTransformer(name="bert-base-uncased", output_dir=args.output_dir, logging_dir=args.log_dir)

  estimator.fit(X=train_texts, y=train_labels)

  probability = estimator.predict(dev_texts)

  predictions = [(1) if prob > 0.5 else 0 for prob in probability]

  accuracy = sklearn.metrics.accuracy_score(dev_labels, predictions)
  f1 = sklearn.metrics.f1_score(dev_labels, predictions)
