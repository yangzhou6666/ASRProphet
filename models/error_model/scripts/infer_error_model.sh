  ...
  do
    ...
    do
      ...
      python3 -u infer_error_model.py \
        ...
        --pretrained_ckpt=$PRETRAINED_CKPTS/error_models/$asr/$accent/seed_"$seed"/best/ErrorClassifierPhoneBiLSTM_V2.pt \
        --output_dir=$PRETRAINED_CKPTS/error_models/$asr/$accent/seed_"$seed"/best \
      > $PRETRAINED_CKPTS/error_models/$asr/$accent/seed_"$seed"/infer_log.txt
      echo
    done
  done
done