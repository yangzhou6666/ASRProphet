...
def parse_args():
    ...
    parser.add_argument("--asr_hypotheses_path", type=str, required=True, help='path to output file containing references and real hypothesis generated by ASR model')
    parser.add_argument("--tts_hypotheses_path", type=str, required=True, help='path to output file containing references and syntheitc hypothesis generated by ASR model')
...
def main(args):
    ...
    asr_hypotheses_path = args.asr_hypotheses_path
    tts_hypotheses_path = args.tts_hypotheses_path

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    print('loading data....')

    asr_data = geneate_error_data_from_hypotheses_file(asr_hypotheses_path)
    tts_data = geneate_error_data_from_hypotheses_file(tts_hypotheses_path)

    data = []
    for asr, tts in zip(asr_data, tts_data):
      assert asr[1] == tts[1]
      error_sequence_asr = asr[0]
      error_sequence_tts = tts[0]
      common_error_sequence = [1 if i == j else 0 for i,j in zip(error_sequence_asr, error_sequence_tts)]
      data.append((common_error_sequence, asr[1]))
