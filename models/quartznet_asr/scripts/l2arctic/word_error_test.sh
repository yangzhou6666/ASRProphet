  ...
  do
    ...
    do
      ...
      do 
        ...
        echo 
        echo
        model_dir=$PRETRAINED_CKPTS/quartznet/finetuned/$accent/$size/seed_"$seed"/word_error_predictor/$sampling
        mkdir -p $model_dir
        python3 -u inference.py \
        --batch_size=64 \
        --output_file=$model_dir/test_out.txt \
        --wav_dir=$WAV_DIR \
        --val_manifest=$DATA/$accent/manifests/train/quartznet/word_error_predictor/$size/$sampling/seed_"$seed"/train.json \
