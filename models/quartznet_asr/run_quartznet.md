...
do
  ...
  do
    echo $accent seed $seed
    CUDA_VISIBLE_DEVICES=5 python3 -u error_model_sampling.py \
...
do
  for seed in 1 2 3
  do
    for accent in "NCC" "TXHC" "YBAA" "YDCK" "YKWK" "ERMS" "MBMPS" "PNV" "TLV"
    do
...
do
  ...
  do
    ...
    do
      ...
      echo $cuda_devices
      echo
      model_dir=$PRETRAINED_CKPTS/quartznet/finetuned/random/$accent/$size/seed_"$seed"
      mkdir -p $model_dir
  ...
  do
    ...
    do
      ...
      CUDA_VISIBLE_DEVICES=$cuda_devices python3 -u finetune.py \
        ...
        --lr=2e-5 \
        --wav_dir=$WAV_DIR \
        --train_manifest=$DATA/$accent/manifests/train/quartznet/word_error_predictor_real/$size/no_word_enhance/seed_"$seed"/train.json \
        --val_manifest=$DATA/$accent/manifests/dev.json \
        --model_toml=$PRETRAINED_CKPTS/quartznet/quartznet15x5.toml \
        --output_dir=$model_dir/recent \
        --best_dir=$model_dir/best \
  ...
  do
    ...
    do
      ...
      CUDA_VISIBLE_DEVICES=$cuda_devices python3 -u finetune.py \
        ...
        --lr=2e-5 \
        --wav_dir=$WAV_DIR \
        --train_manifest=$DATA/$accent/manifests/train/quartznet/asrevolve_error_model_real/$size/seed_"$seed"/train.json \
        --val_manifest=$DATA/$accent/manifests/dev.json \
        --model_toml=$PRETRAINED_CKPTS/quartznet/quartznet15x5.toml \
        --output_dir=$model_dir/recent \
        --best_dir=$model_dir/best \
