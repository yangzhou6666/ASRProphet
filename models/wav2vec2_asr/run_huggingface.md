...
for model in "hubert"
do
  for accent in "YBAA" "ZHAA" "NCC" "TXHC" "YDCK" "YKWK" "ERMS" "MBMPS" "PNV" "TLV"
  do
    echo $model $accent
...
"YBAA" "ZHAA" "NCC" "TXHC" "YDCK" "YKWK" "ERMS" "MBMPS" "PNV" "TLV"
```
for model in "hubert"
do
  for seed in 1 2 3
...
do
  ...
  do
    ...
    do
      for accent in "YKWK" "ERMS" "MBMPS" "PNV" "TLV"
      do
...
do
  ...
  do
    ...
    do
      ...
      do
        echo $accent seed $seed $model
        model_dir=$PRETRAINED_CKPTS/"$model"/finetuned/$accent/$size/seed_"$seed"/word_error_real_mix
        mkdir -p $model_dir
        cuda_devices=5
        CUDA_VISIBLE_DEVICES=$cuda_devices python3 -u finetune.py \
          --wav_dir=$WAV_DIR \
