...
def get_err_score_for_one_position():
    '''
...
if __name__=='__main__':
    ...
    for noun in nouns:
        ...
        for sub in substitues:
            tokens = text.split(" ")
            context = tokens[position - 1] + " " + sub + " " + tokens[position + 1] # concatenate the surrouding words and the substitute
        ...
        score = [sum(n)/len(n) for n in res]
        # sort the substitues according to the score
        substitues, contexts, score = sort(substitues, contexts, score)
        # print the substitues and their scores
        for i in range(len(contexts)):
            print(contexts[i], score[i])
